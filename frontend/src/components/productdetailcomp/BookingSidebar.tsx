...
export const BookingSidebar = ({
  ...
  const refetchAvailability = async () => {
    ...
    if (
      currentProduct?.blockedDates?.some(
        (b: any) =>
          !b.isActive &&
          new Date(b.date).toISOString().slice(0, 10) === iso
      )
    ) {
      setIsDateOk(false);
      setAvailablePackages([]);
    ...
    try {
      ...
      if (!slot || slot.status !== "AVAILABLE") {
        ...
        const firstPkg = (currentProduct.packages ?? []).find(
          (p: any) => (p.maxPeople ?? productCap) >= (adultsCount + childrenCount)
        );
        if (firstPkg) handlePackageSelect(firstPkg.id);
      }
    } catch (err) {
      console.error("Error re-checking availability:", err);
      setIsDateOk(false);
      setAvailablePackages([]);
    } finally {
        ...
        <div className="mb-4 md:mb-6">
          ...
          <div className="flex items-baseline">
            {cheapestPackage ? (
              ...
              <span className="text-2xl md:text-3xl font-bold text-[#ff914d]">
                Contact for pricing
              </span>
            )}
            ...
            >
              <DayPicker
                ...
                classNames={{
                  ...
                  day: "w-10 h-10 flex items-center justify-center ml-1 mt-1 rounded-full hover:ring-2 hover:ring-[#104c57] hover:ring-opacity-50",
          ...
          {!isMobile && showTravellers && (
            ...
            >
              ...
              <button
                ...
                className="w-full mt-2 bg-[#ff914d] text-white rounded-lg py-2"
              >
                Update
              </button>
            </div>
                        ...
                        <div className="flex-1 pr-3">
                          ...
                          {(pkg.description?.length ?? 0) > DESC_LIMIT && (
                            <button
                              ...
                              className="ml-2 text-xs font-medium text-[#ff914d] hover:underline"
                            >
                                  ...
                                  className={`
                                    ...
                                    ${
                                      isSel
                                        ...
                                        : "border-gray-300 hover:border-[#104c57] hover:text-[#104c57]"
                                    }
                                    ${
                                      disabled
              ...
              {selectedPackage && selectedSlotId && selectedTimeSlot && (
                <div className="mt-4">
                  <Link
                    ...
                    className="w-full block text-center bg-[#104c57] text-white px-4 py-2 rounded-lg font-medium hover:bg-[#ff914d]"
          ...
          >
            ...
            <div className="flex-1 overflow-y-auto bg-gray-50 p-4 sm:p-6">
              <div className="flex space-x-3 mb-6 relative">
                ...
                >
                  ...
                  <span className="text-sm font-medium text-gray-900">
                    {adultsCount}A
                    {childrenCount > 0 ? `/${childrenCount}C` : ""}
                  </span>
                </button>
              ...
              {checkingAvail ? (
                <div className="bg-white rounded-lg shadow-sm p-8 text-center">
                  <div className="animate-spin w-8 h-8 mx-auto border-b-2 border-[#ff914d] rounded-full mb-4"></div>
                    ...
                    {slotsForPackage.flatMap((s) => s.Time || []).length > 8 && (
                      <div className="mt-3 text-right">
                        <button
                          ...
                          className="text-[#104c57] hover:text-[#0e3a41] text-sm font-medium"
                        >
            ...
            <footer className="flex items-center justify-end gap-3 p-4 border-t bg-white">
              {selectedPackage && selectedSlot && selectedTimeSlot ? (
                <Link
                  to={`/book/${currentProduct.id}?date=${selectedDateStr}&adults=${adultsCount}&children=${childrenCount}&package=${selectedPackage.id}&slot=${selectedSlotId}&time=${selectedTimeSlot}`}
                  className="bg-[#104c57] text-white px-5 py-2 rounded-lg font-medium hover:bg-[#ff914d]"
                  onClick={() => setShowAvailabilityPopup(false)}
                >
                  Reserve Now
                </Link>
