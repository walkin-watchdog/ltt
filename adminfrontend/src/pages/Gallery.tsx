...
export const Gallery = () => {
  ...
  const fetchFolders = async () => {
    try {
      ...
      const defaultFolders = ['gallery', 'products', 'destinations', 'itineraries', 'experiences', 'team', 'partners', 'slides'];
  ...
  const fetchImages = async (cursor = '', loadMore = false) => {
    ...
    try {
      ...
      if (searchTerm) params.append('prefix', searchTerm);
      ...
      if (response.ok) {
        ...
        setTotalImageCount(loadMore ? prev => prev + data.images.length : data.images.length);
      } else {
  ...
  useEffect(() => {
    fetchImages();
    // Reset selected images when changing folder or search
    setSelectedImages([]);
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
        ...
        {isLoading ? (
          ...
          <div className="text-center py-12">
            ...
            <button
              onClick={handleUploadClick}
              className="bg-[#ff914d] text-white px-4 py-2 rounded-lg hover:bg-[#e8823d] transition-colors"
            >
